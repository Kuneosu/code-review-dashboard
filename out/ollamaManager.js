...
class OllamaManager {
    ...
    async start() {
        ...
        try {
            ...
            this.process.on('exit', (code, signal) => {
                this.outputChannel.appendLine(`[Ollama] 프로세스 종료 (코드: ${code}, 시그널: ${signal})`);
                this.process = null;
            });
            // 서버가 준비될 때까지 대기
    ...
    async waitForServer(maxRetries = 30) {
        for (let i = 0; i < maxRetries; i++) {
            ...
            this.outputChannel.appendLine(`[Ollama] 서버 대기 중... (${i + 1}/${maxRetries})`);
        }
    ...
    async listModels() {
        try {
            ...
            this.outputChannel.appendLine(`[Ollama] 모델 목록 조회 실패: ${error.message}`);
            return [];
        }
    }
    /**
    ...
    async checkModel(modelName) {
        try {
            ...
            const installed = models.some((model) => model.toLowerCase().includes(modelName.toLowerCase()));
            return {
                installed,
                downloading: false,
            };
    ...
    async downloadModel(modelName, onProgress) {
        ...
        return new Promise((resolve, reject) => {
            ...
            pullProcess.on('close', (code) => {
                if (code === 0) {
                    this.outputChannel.appendLine(`[Ollama] ✓ 모델 다운로드 완료: ${modelName}`);
                    resolve();
                }
                else {
            ...
            pullProcess.on('error', (error) => {
                this.outputChannel.appendLine(`[Ollama] 다운로드 프로세스 에러: ${error.message}`);
                reject(error);
            });
        });
    }
    ...
    async autoInstall() {
        ...
        if (platform === 'darwin') {
            ...
            // 다른 OS는 수동 설치 안내
            throw new Error(`${platform}에서는 자동 설치가 지원되지 않습니다. ${this.getInstallGuideUrl()}에서 수동으로 설치해주세요.`);
        }
    }
    /**
     * 현재 실행 중인지 여부
